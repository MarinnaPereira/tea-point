        ...
        {products.map((product) => (
          <div key={product.id} className="relative group">
            ...
            <div className="absolute inset-x-0 bottom-24 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
              <button className="bg-[#c0d1bd] text-white text-[23px] w-full px-10 py-6">
                Add to Cart
              </button>
            </div>
            <h3 className="text-[25px] text-[#444444] pt-5">{product.productName}</h3>
            <p className="text-[20px] text-[#444444]">{product.price.toFixed(2)} Euro / 100g</p>
          </div>
        ))}
      </div>
    </>
